...
- `/group-buys`：總公司可查看全部團購並建立新團；分店目前只會看到指派給自己門市的團購。
- `/group-buys/new`：總公司建立一團一商品的草稿團購，選擇參與門市並設定預設取貨時間。
- `/group-buys/[id]`：總公司查看團購完整資料、各參與門市取貨時間與目前狀態。
- `/group-buys/[id]/edit`：總公司可修改團購內容、參與門市與各門市取貨時間。
- `GET`、`POST /api/group-buys`：依角色讀取團購資料，或由總公司建立草稿團購。
- `PATCH /api/group-buys/[id]`：僅總公司可更新團購及參與門市設定；若參與門市已有訂單，系統會阻止直接移除該門市。
- `POST /api/group-buys/[id]/status`：僅總公司可執行發布、暫停或結束；發布前必須至少選擇一家參與門市。
